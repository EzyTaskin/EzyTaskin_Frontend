...
import React, { useState } from "react";
import { Star } from "lucide-react";
>>>>>>> 62eded6 (Update the search function)
...
export default function ServiceCards({ providers }: { providers: any[] }) {
  ...
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentItems = providers.slice(startIndex, startIndex + ITEMS_PER_PAGE);

  return (
    <div className="px-20 text-left">
      <h2 className="text-2xl font-semibold my-8">
        Showing <span className="font-bold">{providers.length}</span> service
        cases
      </h2>
        ...
        {currentItems.map((provider, index) => (
          ...
          >
            <div className="space-y-3">
              ...
              <p className="text-sm text-gray-500">📍 {provider.location}</p>
              <div className="flex flex-wrap gap-2">
                {provider.tags.map((tag: string, i: number) => (
                  <span
                    key={i}
                    className="bg-gray-100 px-2 py-1 text-xs rounded-full border"
                  >
